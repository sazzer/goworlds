...
impl<F> ServerConfigurer for F where F: Sync + Send + Fn(&mut ServiceConfig) {
  fn configure_server(&self, cfg: &mut ServiceConfig) {
    self(cfg);
  }
}
...
mod tests {
  ...
  fn test_register_closure() {
    Server::new(3000)
      .add_configuration(Arc::new(|_: &mut ServiceConfig| {
      }));
  }
  ...
  fn test_register_free_function() {
    Server::new(3000)
      .add_configuration(Arc::new(register_free_function));
  }
